...
def getScores(correct_answers, student_choices):
    score = 0
    for key, value in student_choices.items():
        correct_answer = correct_answers[key]
        if correct_answer == value:
            score += 1
        else:
            print('Questão ', key, ' está errada. Resposta correta é ', correct_answer, ' mas a resposta do aluno é ', value)
            pass
